...
int readADC(int i2c_handle,uint8_t multiplexer, uint8_t rate, uint8_t gain)
{
	...
	if(write(i2c_handle, config,3) != 3) return -1; //  error writing

	// waiting for data ready escape after 2 seconds
	start = time(NULL);

	if( write(i2c_handle, config,1) != 1) return -2;
	...
	while(1)
	{
		...
		if(read(i2c_handle,&config[1],1) != 1) return -3; //  error reading

		if(config[1] & 0x80)
		{
			config[0]=REG_CONV;
			if( write(i2c_handle, config,1) != 1) return -4;
			if(read(i2c_handle,&config[1],2) != 2) return -5; //  error reading
...
int main (int argc, char*argv[])
{
	...
	int i2c_handle = open("/dev/i2c-3",O_RDWR);
	if(i2c_handle <0)
	{
	...
	InstrumentationDriver* boardDriver;
	if(argc==2)
	{
		if(strcmp(argv[1], "0x48")==0)
		{
			boardDriver = InstrumentationDriver_create(0x48);
		}
		else if(strcmp(argv[1], "0x49")==0)
		{
			boardDriver = InstrumentationDriver_create(0x49);
		}
		else
		{
			printf("Endereço I2C desconhecido.\n");
			return EXIT_FAILURE;
		}
	}
	else
	...
	for(r=0;r<8;r++)
	{
		...
		printf("---> got  %d Samples/sec with avg value of %.1f, last value 0x%X\n", i / diff,1.0 * avgValue / i, value);
	}
